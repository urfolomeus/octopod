...
class DataFetcher:
  def fetch_electricity(self, start_date):
    mpan = os.environ["OCTOPUS_MPAN"]
    serial = os.environ["OCTOPUS_SERIAL_ELEC"]
    url = f"{BASE_URL}/electricity-meter-points/{mpan}/meters/{serial}/consumption?period_from={start_date}&page_size={PAGE_SIZE}"
